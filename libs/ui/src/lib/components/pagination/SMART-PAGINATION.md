...
```typescript
export type PaginationItem = { type: 'page'; value: number } | { type: 'ellipsis'; value: string };
```
...
- **`pages()`** - Array of PaginationItem for rendering with @for loops
...
3. **Type Safety**
   - PaginationItem type ensures correct usage
